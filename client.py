...
def main(host, port1, port2):
	...
	stt_au = audio.open(format=FORMAT, channels=CHANNELS, rate=RATE, input=True, output=True, frames_per_buffer=CHUNK)
	...
	sa.daemon = True
	sa.start()
	ta = threading.Thread(target = stt, args=(voiceSocket, stt_au))
...
    tts.save(s_path + 'Csound' + str(index) + '.mp3')
    time.sleep(1)
    try:
        initMixer()
        filename= s_path + 'Csound' + str(index) + '.mp3'
        playmusic(filename)
        index = index + 1
...
    tts = gTTS(text=sd, lang='ko')
    tts.save(s_path + 'CgetTier' + str(index) + '.mp3')

    try:
        initMixer()
        filename = s_path + 'CgetTier' + str(index) + '.mp3'
        playmusic(filename)
        index = index + 1
    except KeyboardInterrupt:  # to stop playing, press "ctrl-c"
        stopmusic()
